...
// The 8 sort keys buildCarOrderBy accepts.
export const NL_SORT_KEYS = [
  "newest", "oldest", "priceAsc", "priceDesc",
  "yearAsc", "yearDesc", "mileageAsc", "mileageDesc",
];

const nlInt = () => z.number().int().nullable();

// The AI's structured-search output contract. Kept flat (no nested objects, no
// unions beyond nullable scalars) so the JSON Schema Gemini receives stays small.
// Enums are generated from car-options so the schema can't drift from the DB.
// Off-vocabulary values shouldn't reach here (Gemini is constrained by this same
// schema as responseSchema); if they do, run.js re-validation falls back safely.
export const carSearchFiltersSchema = z.object({
  make: z.array(z.string()).default([]),
  bodyType: z.array(z.enum(BODY_TYPES)).default([]),
  fuelType: z.array(z.enum(FUEL_TYPES)).default([]),
  transmission: z.array(z.enum(TRANSMISSIONS)).default([]),
  color: z.string().nullable(), // a single colour name, e.g. "red"
  minSeats: nlInt(),
  minPrice: nlInt(),
  maxPrice: nlInt(),
  minYear: nlInt(),
  ...
  minMileage: nlInt(),
  maxMileage: nlInt(),
  sortBy: z.enum(NL_SORT_KEYS).nullable(),
  search: z.string().nullable(), // ONE model name only, never an adjective
  interpretation: z.string().default(""),
  unmapped: z.array(z.string()).default([]),
  confidence: z.number().min(0).max(1).default(0),
});

// Bounds the public NL-search input: the 200-char cap is the primary cost control
// and a prompt-injection blast-radius limiter.
