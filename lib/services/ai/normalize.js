...
function toInt(v, min = null) {
    // Absent must stay absent: the model returns explicit null for fields it didn't
    // fill, and Number(null) === 0 — so without this guard a null year became 0,
    // which clampYear pinned to 1900 (an unasked-for filter that returns no cars).
