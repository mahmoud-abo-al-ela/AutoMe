...
export async function startConversation(userId, carId = null) {
    ...
    const existingConversation = await messageRepository.findExistingConversation(participantIds, carId);
...
export async function getUserConversations(userId, pagination = {}) {
    ...
    const result = await messageRepository.findUserConversations(user.id, pagination);
    const unreadCounts = await messageRepository.getUnreadCountPerConversation(user.id);
