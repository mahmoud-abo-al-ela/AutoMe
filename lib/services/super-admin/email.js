...
export async function sendOrganizationInvitationEmail({
  ...
  if (!process.env.EMAILJS_PUBLIC_KEY || !process.env.EMAILJS_PRIVATE_KEY || !process.env.EMAILJS_SERVICE_ID) {
    throw new Error(
      "EmailJS is not configured. Please add EMAILJS_PUBLIC_KEY, EMAILJS_PRIVATE_KEY, EMAILJS_SERVICE_ID, and EMAILJS_TEMPLATE_ID to your .env file. Get your credentials from https://dashboard.emailjs.com"
    );
  }

  const baseUrl = process.env.NEXT_PUBLIC_URL || 'http://localhost:3000';

  // Construct path-based URL for organization
  const orgUrl = `${baseUrl}/org/${organizationSlug}`;
    ...
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        ...
        : `<p>To get started, create your account using this email address:</p>
             <a href="${orgUrl}/sign-up?redirect_url=${encodeURIComponent('/admin')}" 
                style="display: inline-block; background-color: #0070f3; color: white; padding: 12px 24px; text-decoration: none; border-radius: 6px; margin-top: 16px;">
               Create Account
             </a>
