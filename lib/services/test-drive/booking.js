...
export async function editTestDrive(testDriveId, updateData, userId) {
  ...
  const existingTestDrive = await testDriveRepository.findTestDriveById(
    testDriveId
  );
  ...
  if (existingTestDrive.userId !== user.id && !isAdmin && !hasOrgAccess) {
...
export async function cancelTestDrive(testDriveId, userId) {
  ...
  if (testDrive.userId !== user.id && !isAdmin && !hasOrgAccess) {
