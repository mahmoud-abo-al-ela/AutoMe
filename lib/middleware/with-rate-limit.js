import aj, { ajNlSearch } from "@/lib/arcjet";
...
/**
 * Enforce the dedicated public natural-language search bucket, so abuse of the
 * AI endpoint can't starve dealer flows. Must be called OUTSIDE any
 * unstable_cache (it reads request headers via Arcjet).
 */
export async function enforceNlSearchRateLimit(requested = 1) {
  return enforce(ajNlSearch, requested);
}
